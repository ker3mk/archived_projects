...
';
	...
	$q="SELECT * FROM news,newstype where news.newstype=newstype.id order by news.id desc limit $k,$dis";
...
<input type="hidden" name="silinecekdegerresim" value="'.$row['newsimage'].'">
<input type="submit" name="sil" value="Sil" > 
</form>';}

else if( $_SESSION['admin']==1){
$qa="SELECT Count(*) FROM favoritenews where favoritenews.userid=".$_SESSION['userid']." and favoritenews.newsid=".$myid.";";

	$r=mysqli_query($con,$qa);
	$ror=mysqli_fetch_array($r);
	if(!$ror[0]>0){
echo '
<form method="post" action="'.$_SERVER['PHP_SELF'].'">
<input type="hidden" name="postala" value="'.$myid.'">
<input type="submit" name="kullanicayaekle" value="Favorim Yap" > 
</form>';}
else{
echo '* Favorilerinize Eklidir ';}}}
echo'
</div>
&nbsp;&nbsp;
</div>
...
$query3="SELECT COUNT(*) FROM news where news.id=".$sayi.";";
  $query4 = mysqli_query($con, $query3);
  $row4=mysqli_fetch_array($query4);
  $counter=$row4[0];
  if($counter>0){
  
$query2="SELECT * FROM news,newstype where news.newstype=newstype.id and news.id=".$sayi.";";
	$result2=mysqli_query($con,$query2);
		while($row = mysqli_fetch_array($result2)){
			echo'<div  class="newsdiv" >
<div class="newh"style="margin-top:10px;" align="center" ><h3>'.$row['header'].'</h3>&nbsp;<hr>&nbsp;</div>
<div align="center"  > <img src="'.$row['newsimage'].'" alt="" /> </div>

<div style="margin-left:50px;margin-right:50px;margin-top:10px;">'.$row['content'].'</br>&nbsp;</div><hr>&nbsp;
<div>&nbsp;&nbsp;Haber Türü: '.$row['name'].'</br>
&nbsp;&nbsp;Tarih: '.$row['date'].'</div>
&nbsp;&nbsp;
</div>
&nbsp;';
...
if(isset($_POST['kullanicayaekle'])&&isset($_POST['postala'])){
$myfv="insert into  favoritenews (userid, newsid) VALUES ("."'".$_SESSION['userid']."',".$_POST['postala'].");";
	mysqli_query($con,$myfv);

}
if(isset($_POST['silinecekdeger'])&&isset($_POST['sil'])){
$mysil="Delete from news where news.id=".$_POST['silinecekdeger'].";";
